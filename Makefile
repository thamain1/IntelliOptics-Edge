.PHONY: install install-lint install-pre-commit test test-with-docker test-all \
	lint format test-with-k3s-setup-ee test-with-k3s-helm validate-setup-ee \
	validate-setup-helm helm-install helm-package helm-local
SHELL := /bin/bash
...
# For example:
#   make helm-install HELM_ARGS="--set intelliopticsApiToken=api_2hRQVo...."
HELM_ARGS :=
...
helm-install:
	helm upgrade -i -n default $(HELM_TOKEN_ARG) $(HELM_ARGS) $(HELM_RELEASE_NAME) deploy/helm/intellioptics-edge-endpoint/intellioptics-edge-endpoint

helm-package:
	helm package deploy/helm/intellioptics-edge-endpoint/intellioptics-edge-endpoint

# TODO: update this with inference server support
helm-local:
	helm upgrade -i -n default $(HELM_TOKEN_ARG) $(HELM_ARGS) --set=edgeEndpointTag=dev $(HELM_RELEASE_NAME) deploy/helm/intellioptics-edge-endpoint/intellioptics-edge-endpoint
