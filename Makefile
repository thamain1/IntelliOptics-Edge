...
helm-install:
    helm upgrade -i -n default ${HELM_ARGS} ${HELM_RELEASE_NAME} deploy/helm/intellioptics-edge-endpoint

helm-package:
    helm package deploy/helm/intellioptics-edge-endpoint

# TODO: update this with inference server support
helm-local:
    helm upgrade -i -n default ${HELM_ARGS} --set=edgeEndpointTag=dev ${HELM_RELEASE_NAME} deploy/helm/intellioptics-edge-endpoint
