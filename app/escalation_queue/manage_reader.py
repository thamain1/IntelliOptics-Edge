...
from intellioptics import IntelliOptics, IntelliOpticsClientError, ImageQuery
...
@lru_cache(maxsize=1)
def _intellioptics_client() -> IntelliOptics:
    """Returns an IntelliOptics client instance with EE-wide credentials for escalating from the queue."""
    # Don't specify an API token here - it will use the environment variable.
    return IntelliOptics()  # NOTE this will wait the default 10 seconds when there's no connection.
...
def _escalate_once(  # noqa: PLR0911
    ...
    try:
        io_client = _intellioptics_client()
    except IntelliOpticsClientError as ex:
        # We don't catch API token related exceptions here, since we want those to visibly fail. An
        # IntelliOpticsClientError exception will be raised for other kinds of errors, including when the client could
        # not be created due to no internet connection.
        logger.info(f"Got error {ex=} while trying to create the IntelliOptics client. Will retry.")
...
if __name__ == "__main__":
    ...
    # the IntelliOptics SDK will automatically retry the request, sending the same query parameters to the edge afresh
