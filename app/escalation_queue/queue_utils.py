...
def safe_escalate_with_queue_write(
    ...
    try:
        ...
        # escalation queue process will handle these errors and skip the escalation if it can't succeed, so we can
        # safely write it to the queue no matter what the exception here was.
        logger.info(
            "Writing an escalation for detector {detector_id} to the queue because there was an exception while "
            f"escalating: {ex=}."
        )
