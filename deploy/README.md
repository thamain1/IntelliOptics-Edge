...
You'll also need to provide credentials for your Azure Container Registry (ACR). Export the login server, username, and password for the registry so they are available to the deployment scripts, for example:
```bash
export ACR_LOGIN_SERVER="myregistry.azurecr.io"
export AZURE_REGISTRY_USERNAME="<service-principal-or-admin-user>"
export AZURE_REGISTRY_PASSWORD="<service-principal-password>"
```
If you prefer to authenticate using the Azure CLI, make sure `az login` has been run beforehand and that you have rights to push to the registry.
...
Check the `refresh_creds` cron job to see if it's running. If it's not, you may need to re-run [refresh-acr-login.sh](/deploy/bin/refresh-acr-login.sh) to update the credentials used by docker/k3s to pull images from ACR.  If the script is running but failing, this indicates that the stored Azure credentials (in secret `azure-registry-credentials`) are invalid or not authorized to pull algorithm images from ACR.
