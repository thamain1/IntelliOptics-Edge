...
# Put a specific tag on an existing image in Azure Container Registry
# Assumptions:
...
ACR_NAME=${AZURE_CONTAINER_REGISTRY:-${ACR_NAME:-acrintellioptics}}
ACR_LOGIN_SERVER=${AZURE_REGISTRY_SERVER:-${ACR_LOGIN_SERVER:-${ACR_NAME}.azurecr.io}}
...
if ! command -v az >/dev/null 2>&1; then
    echo "Azure CLI (az) is required to retag images" >&2
    exit 1
fi

if ! command -v jq >/dev/null 2>&1; then
    echo "jq is required to retag images" >&2
    exit 1
fi
